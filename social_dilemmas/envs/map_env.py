...
def rgb2gray(rgb, gray_scale=True):
    if gray_scale:
...
class MapEnv(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents.values():
            # check if agent is out of game before removing from the color map
            if not agent.is_out_of_game():
        ...
        for agent in self.agents.values():
            # check if agent is out of game before consuming
            if not agent.is_out_of_game():
                pos = agent.pos
        ...
        for agent in self.agents.values():
            # check if agent is out of game before adding to the color map
            if not agent.is_out_of_game():
                row, col = agent.pos[0], agent.pos[1]
    ...
    def full_map_to_colors(self):
        map_with_agents = self.get_map_with_agents()
        rgb_arr = np.zeros((map_with_agents.shape[0], map_with_agents.shape[1], 3))
        return self.map_to_colors(map_with_agents, self.color_map, rgb_arr).astype(np.uint8)

    def agent_id_to_int(self, agent):
        return int(agent.agent_id.split('-')[-1])
    ...
    def update_map_fire(
        ...
        if beam_width == 1:
            ...
            firing_pos = [
                start_pos + right_shift - firing_direction,
                start_pos + (2*right_shift) - firing_direction,
                start_pos,
                start_pos - right_shift - firing_direction,
                start_pos - (2*right_shift) - firing_direction,
            ]
