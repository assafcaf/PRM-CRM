...
def task_by_name(name, original_name=None, short=False, max_episode_steps=500,
    ...
    if name == "reacher":
        ...
        try:
            ...
            # See "make_env"
            print("Trying to make an environment from original name: {}".format(original_name))
            env = gym.make(original_name)
            # env = AtariPreprocessing(env, screen_size=84, grayscale_obs=True, scale_obs=True, frame_skip=1)
            ...
            env = limit(t=max_episode_steps, env=env)
            return env
        except Exception:
...
class CompressedPixelViewer(TransparentWrapper):
    ...
    def _preprocess(self, image):
        ...
        image = image[25:, 10:-10]  # Crop
        image = cv2.resize(image, dsize=(84, 84), interpolation=cv2.INTER_CUBIC)
...
class ObservationStack(VecEnv):
    def __init__(self, env, stacked_frames, n_envs=1):
        self.observation_space = gym.spaces.Box(
            low=0,
            high=255,
            shape=(84, 84, stacked_frames),
            dtype=np.uint8,
        )
        super().__init__(num_envs=n_envs,
                         observation_space=self.observation_space,
                         action_space=env.action_pace)
